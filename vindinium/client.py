...
class Client(object):
    ...
    def run(self, bot):
        ...
        try:
            ...
            while not finished:
                ...
                turnTime = (time.clock() - t0) * 1000
                state = self.__move(play_url, action)
                finished = state['game']['finished']
                print 'turn {} of {} took {:.1f}ms'\
                    .format(state['game']['turn'], state['game']['maxTurns'], turnTime)
    ...
    def __connect(self):
        ...
        r = self.__session.post(server + endpoint, params, timeout=100*60)
